...
func (s *Server) AddRoutes(mux *server.Server, tunnelBridge http.Handler) {
	...
	mux.HandleFunc("GET /api/custom-groups", wrap(s.listCustomGroups))
	mux.HandleFunc("POST /api/custom-groups", wrap(s.createCustomGroup))
	mux.HandleFunc("GET /api/custom-groups/{id}", wrap(s.getCustomGroup))
	mux.HandleFunc("PATCH /api/custom-groups/{id}", wrap(s.updateCustomGroup))
	mux.HandleFunc("DELETE /api/custom-groups/{id}", wrap(s.deleteCustomGroup))
	mux.HandleFunc("PUT /api/custom-groups/{id}/members", wrap(s.setCustomGroupMembers))
